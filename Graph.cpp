...
Graph::Graph() {
    // Initialize with an empty adjacency matrix 
    Matrix.clear(); // Clear the matrix, ensuring it starts empty
}
...
// Check if the matrix is valid (it's a square matrix)
...
void Graph::operator+=(const Graph& g1) {
    if (!isSameSize(g1.getMatrix(), this->getMatrix())) {
        throw std::invalid_argument("the matrices are not equal in size"); //if they are not the same size cant sum
    }
...
bool Graph::operator>(Graph& g1) {
    ...
    else if (g1.isContaine(*this) && !isSameSize(g1.getMatrix(), this->getMatrix())) {
        return true;
    }
    return false;
}
